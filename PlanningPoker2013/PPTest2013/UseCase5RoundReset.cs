...
{
    public class UseCase5RoundReset
    {
        [TestClass]
        public class ResetRound
        {
            ...
            [TestMethod]
            public void ResetWithoutRound()
            {
                ResetWithoutRoundAsync().GetAwaiter().GetResult();
            }

            async Task ResetWithoutRoundAsync()
            {
                var ct = createdTable();
                ...
                try
                {
                    ...
                    await ct.Table.ResetRound();
                }
                catch (PPNoCurrentRoundException)
                {
                    return;

                }
                Assert.IsTrue(false, "is should ginve a error when no round");
            }

            [TestMethod]
            public void ResetSecondRound()
            {
                ResetSecondRoundAsync().GetAwaiter().GetResult();
            }
            async Task ResetSecondRoundAsync()
            {
                var ct = createdTable();
